 ...
 *
 * <p>Each test states a business situation and asserts the exact number of penalties.
 * Exact counts rather than "at least one" matter here: a constraint that fires twice
 * for one violation silently outweighs the others and skews every plan the solver produces.
 */
class HardConstraintTest {
    ...
    private final ConstraintVerifier<ShiftScheduleConstraintProvider, ShiftSchedule> verifier =
            ConstraintVerifier.build(
                    new ShiftScheduleConstraintProvider(),
                    ShiftSchedule.class,
                    ShiftSlot.class);

    private static final ConstraintWeights WEIGHTS =
        ...
        verifier
                .verifyThat(
                        ShiftScheduleConstraintProvider
                                ::noOverlappingShifts)
                .given(
                        slot(saturdayBar, 0, anna),
                        slot(sundayEarly, 0, anna))
                .penalizesBy(1);
    }
    ...
    void anEmployeeCannotBeScheduledAtALocationTheyAreNotClearedFor() {
        ...
        verifier
                .verifyThat(
                        ShiftScheduleConstraintProvider
                                ::employeeMustBeClearedForLocation)
    ...
    void aShiftMissingItsRequiredSkillIsPenalizedOncePerMissingHolder() {
        ...
        verifier
                .verifyThat(
                        ShiftScheduleConstraintProvider
                                ::requiredSkillsMustBeCovered)
                .given(
                        slot(barShift, 0, withoutBar),
                        slot(barShift, 1, withoutBar))
                .penalizesBy(1);
    }
        ...
        verifier
                .verifyThat(
                        ShiftScheduleConstraintProvider
                                ::requiredSkillsMustBeCovered)
                .given(
                        slot(barShift, 0, bartender),
                        slot(barShift, 1, other))
                .penalizesBy(0);
    }
    ...
    void exceedingTheWeeklyMaximumIsPenalizedByTheExcessMinutes() {
        ...
        verifier
                .verifyThat(
                        ShiftScheduleConstraintProvider
                                ::maximumWeeklyHoursMustNotBeExceeded)
                .given(
                        slot(saturday, 0, anna),
                        slot(sunday, 0, anna))
                .penalizesBy(120);
    }
    ...
    void tooLittleRestBetweenTwoShiftsIsPenalizedByTheMissingMinutes() {
        ...
        verifier
                .verifyThat(
                        ShiftScheduleConstraintProvider
                                ::minimumRestBetweenShiftsMustBeRespected)
                .given(
                        WEIGHTS,
                        slot(saturdayEvening, 0, anna),
                        slot(sundayMorning, 0, anna))
                .penalizesBy(120);
    }
    ...
    void exceedingTheDailyMaximumIsPenalizedByTheExcessMinutes() {
        ...
        verifier
                .verifyThat(
                        ShiftScheduleConstraintProvider
                                ::maximumDailyHoursMustNotBeExceeded)
                .given(
                        WEIGHTS,
                        slot(morning, 0, anna),
                        slot(evening, 0, anna))
